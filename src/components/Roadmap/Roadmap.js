...
const RoadmapItem = ({ phase, content, src, ...rest }) => {
    return (
        <>
            ...
            <Box mx={{ base: 4, md: 12, lg: 24, xl: 48 }} align='center'>
                ...
                <Grid templateColumns={{ base: 'repeat(4, 1fr)', md: `repeat(${n_columns}, 1fr)` }} templateRows='repeat(14)' rowGap={{ base: 4, md: 4 }}>
                    ...
                    <GridItem rowStart={1} rowSpan={{ base: 1, md: 3 }} colStart={1} colSpan={item_w}>
                        <RoadmapItem phase='Merchandise' content={['Free ITU hoodie for holders.']} src={roadmap1} />
                    </GridItem>

                    {/* 2 */}
                    <GridItem rowStart={{ base: 2, md: 3 }} rowSpan={{ base: 1, md: 3 }} colStart={{ base: 1, md: item_start }} colSpan={item_w}>
                        <RoadmapItem phase='Charity Drive/Donation' content={['We will list a few charity organization and holders will choose one to whom they wish to donate.']} src={roadmap2} />
                    </GridItem>

                    {/* 3 */}
                    <GridItem rowStart={{ base: 3, md: 5 }} rowSpan={{ base: 1, md: 3 }} colStart={1} colSpan={item_w}>
                        <RoadmapItem phase='Real Life Event for ITU holders' content={['ITU food fiesta & fun fair(location to be confirmed)', 'ITU Holders gathering, meet & dine.']} src={roadmap3} />
                    </GridItem>

                    {/* 4 */}
                    <GridItem rowStart={{ base: 4, md: 7 }} rowSpan={{ base: 1, md: 3 }} colStart={{ base: 1, md: item_start }} colSpan={item_w}>
                        <RoadmapItem phase='Acquire a MetaVerse land' content={['As the buzz around the metaverse picks up pace, the virtual land opportunity takes centre stage, we wouldn’t want to miss out the opportunity and we will purchase a top tier metaverse land for ITU for future adoption.']} src={roadmap4} />
                    </GridItem>

                    {/* 5 */}
                    <GridItem rowStart={{ base: 5, md: 9 }} rowSpan={{ base: 1, md: 3 }} colStart={1} colSpan={item_w}>
                        <RoadmapItem phase='ITU Food Chain or Merchandise retail shop.' content={['ITU Food concept restaurant headed by our founder celebrity chef bob.', 'ITU merchandise collabs with known brands.', 'Part of sales proceed will go back to the community wallet.']} src={roadmap5} />
                    </GridItem>

                    {/* 6 */}
                    <GridItem rowStart={{ base: 6, md: 11 }} rowSpan={{ base: 1, md: 3 }} colStart={{ base: 1, md: item_start }} colSpan={item_w}>
                        <RoadmapItem phase='2nd NFT Drop' content={['Companion drop for NFT holders']} src={roadmap6} />
                    </GridItem>

                    {/* 7 */}
                    <GridItem rowStart={{ base: 7, md: 13 }} rowSpan={{ base: 1, md: 3 }} colStart={1} colSpan={item_w}>
                        <RoadmapItem phase='Roadmap 2.0' content={['Improvisation of roadmap 1.0, dao governance, tokenisation and nft/token staking are one of the few that we are working hard to make it a realisation.']} src={roadmap7} />
                    </GridItem>
